...
const NABHComplianceChecklist: React.FC = () => {
	...
	const [selectedStandard, setSelectedStandard] = useState<NABHStandard | null>(
		null,
	);
	...
	const submitAssessment = async () => {
		...
		try {
			...
			const findings = Object.entries(assessmentData).map(
				([itemId, data]: [string, any]) => ({
					itemId,
					status: data.status,
									...
									<div className="space-y-3">
										<div>
											...
											<div className="flex space-x-4 mt-1">
												{["COMPLIANT", "PARTIAL", "NON_COMPLIANT"].map(
													(status) => (
														<label
															key={status}
														...
														>
															<Checkbox
																checked={
																	assessmentData[item.id]?.status === status
																}
																onCheckedChange={() =>
																	updateChecklistItem(
																		item.id,
