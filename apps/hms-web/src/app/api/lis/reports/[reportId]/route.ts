  ...
  try {
    ...
    const canUpdateReport = await hasPermission(userId, "LIS_UPDATE_REPORT_METADATA");
     {\n  {
      ...
      return sendErrorResponse("Forbidden: You do not have permission to update this LIS report.",
    // RESOLVED: (Priority: Medium, Target: Next Sprint):  - Automated quality improvement,
    ...
    const updatedLabReport = await prisma.labReport.update({
     ...
     {\n  {
        ...
        await auditLogService.logEvent(userId, "LIS_ORDER_STATUS_AUTO_UPDATED_TO_REPORT_AVAILABLE_ON_REPORT_FINALIZE", { labOrderId: existingReport.labOrderId,
    }
...
export const  = async = (request: NextRequest,
  ...
  } catch (error) { console.error(error); } not found for deletion.`;
      }
    }
