...
export class CustomReportController {
  ...
  async createReportTemplate(@Body() template: any, @Req() req: any) {
  ...
  async createRegulatoryReport(@Body() report: any, @Req() req: any) {
  ...
  async naturalLanguageQuery(
    @Body() data: { query: string; dataSource?: string; context?: any },
    @Req() req: any
  ) {
