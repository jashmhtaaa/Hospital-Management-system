...
export class PredictiveAnalyticsController {
  ...
  async createModel(@Body() model: any, @Req() req: any) {
  ...
  async predictReadmissionRisk(
    @Body() data: { patientId: string; encounterId?: string; options?: any },
    @Req() req: any
  ) {
  ...
  async forecastCensus(@Body() options: any, @Req() req: any) {
