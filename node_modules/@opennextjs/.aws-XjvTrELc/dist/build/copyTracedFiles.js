...
function isExcluded(srcPath) {
    return EXCLUDED_PACKAGES.some((excluded) => srcPath.match(getCrossPlatformPathRegex(`/node_modules/${excluded}/`)));
}
...
export function getManifests(nextDir) {
    return {
...
export async function copyTracedFiles({ buildOutputPath, packagePath, outputDir, routes, bundledNextServer, skipServerFiles, }) {
     ...
     */
    const processNftFile = (nftFile) => {
        const subDir = path.dirname(nftFile);
        const files = JSON.parse(readFileSync(path.join(dotNextDir, nftFile), "utf8")).files;
        files.forEach((tracedPath) => {
...
--------------------------------------------------------------------------------`);
        ...
        filesToCopy.set(path.join(standaloneNextDir, serverPath), path.join(outputNextDir, serverPath));
    };
    ...
    if (existsSync(path.join(dotNextDir, MIDDLEWARE_TRACE_FILE))) {
        // We still need to copy the nft.json file so that computeCopyFilesForPage doesn't throw
        copyFileSync(path.join(dotNextDir, MIDDLEWARE_TRACE_FILE), path.join(standaloneNextDir, MIDDLEWARE_TRACE_FILE));
    ...
    readdirSync(standaloneNextDir)
        ...
        .forEach((file) => copyFileSync(path.join(standaloneNextDir, file), path.join(outputNextDir, file)));
    ...
    readdirSync(standaloneServerDir)
        ...
        .forEach((file) => copyFileSync(path.join(standaloneServerDir, file), path.join(path.join(outputNextDir, "server"), file)));
    // Copy patch file
    ...
    if (hasPageDir) {
        ...
        const staticFiles = Object.values(loadPagesManifest(standaloneNextDir));
