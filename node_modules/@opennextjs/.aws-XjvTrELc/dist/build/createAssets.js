...
export function createCacheAssets(options) {
    ...
    buildHelper.removeFiles(outputPath, ({ relativePath }) => relativePath.endsWith(".js") ||
    ...
    buildHelper.traverseFiles(outputPath, () => true, ({ absolutePath }) => {
        ...
        const cacheFileContent = {
            ...
            body: files.body
                ? fs
                    ...
                    .toString(isBinaryContentType(cacheFileMeta.headers["content-type"])
                    ? "base64"
                    : "utf8")
                : undefined,
        };
    ...
    const fetchCachePath = path.join(appBuildOutputPath, ".next/cache/fetch-cache");
    ...
    if (!options.config.dangerous?.disableTagCache) {
        ...
        buildHelper.traverseFiles(outputPath, ({ absolutePath }) => absolutePath.endsWith(".meta"), ({ absolutePath, relativePath }) => {
            const fileContent = fs.readFileSync(absolutePath, "utf8");
            const fileData = JSON.parse(fileContent);
            if (fileData.headers?.["x-next-cache-tags"]) {
                fileData.headers["x-next-cache-tags"]
                    ...
                    metaFiles.push({
                        ...
                        path: {
                            S: path.posix.join(buildId, relativePath.replace(".meta", "")),
                        },
        ...
        if (metaFiles.length > 0) {
            ...
            fs.writeFileSync(path.join(providerPath, "dynamodb-cache.json"), JSON.stringify(metaFiles));
        }
    }
    // We need to remove files later because we need the metafiles for dynamodb tags cache
    buildHelper.removeFiles(outputPath, ({ relativePath }) => !relativePath.endsWith(".cache"));
