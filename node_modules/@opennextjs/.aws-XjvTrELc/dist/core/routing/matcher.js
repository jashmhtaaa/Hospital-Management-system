...
import { constructNextUrl, convertFromQueryString, convertToQueryString, escapeRegex, getUrlParts, isExternal, unescapeRegex, } from "./util";
const routeHasMatcher = (headers, cookies, query) => (redirect) => {
    switch (redirect.type) {
        case "header":
            return (!!headers?.[redirect.key.toLowerCase()] &&
                new RegExp(redirect.value ?? "").test(headers[redirect.key.toLowerCase()] ?? ""));
        case "cookie":
            return (!!cookies?.[redirect.key] &&
                new RegExp(redirect.value ?? "").test(cookies[redirect.key] ?? ""));
        case "query":
            return query[redirect.key] && Array.isArray(redirect.value)
                ? redirect.value.reduce((prev, current) => prev || new RegExp(current).test(query[redirect.key]), false)
...
function convertMatch(match, toDestination, destination) {
    if (!match) {
        return destination;
    }
    const { params } = match;
...
export function getNextConfigHeaders(event, configHeaders) {
    if (!configHeaders) {
        return {};
    }
...
export function handleRewrites(event, rewrites) {
...
export function handleRedirects(event, redirects) {
...
export function fixDataPage(internalEvent, buildId) {
...
export function handleFallbackFalse(internalEvent, prerenderManifest) {
    ...
    const routesAlreadyHaveLocale = locales?.includes(rawPath.split("/")[1]) ||
