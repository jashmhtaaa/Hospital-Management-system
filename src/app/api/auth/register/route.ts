...
const RegisterSchema = z.object({
  ...
  role_name: z
    .enum(['Admin', 'Doctor', 'Nurse', 'Receptionist', 'Lab Technician', 'Pharmacist', 'Patient'])
    .default('Patient'),
});
...
export const _POST = async (request: Request) => {
  try {
    ...
    // 1. Get Role ID
    const roleResult = await DB.prepare('SELECT role_id FROM Roles WHERE role_name = ?')
      .bind(role_name)
    ...
    return new Response(
      JSON.stringify({ message: 'User registered successfully', userId: newUserId }),
      {
        status: 201, // Created
        headers: { 'Content-Type': 'application/json' },
      }
