...
// Execute query;
const rangesResult = await database_1.DB.query(query, parameters);
...
const testCheckResult = await database_1.DB.query();
...
if (!session.user) {
    ...
    body.test_id,
        ...
        body.age_low, body.age_high;
    ;
    const overlapResult = await database_1.DB.query(overlapCheckQuery, overlapCheckParams);
    if (!session.user) {
        return NextResponse.json();
        {
...
;
const result = await database_1.DB.query(insertQuery, insertParameters);
...
const fetchQuery = `;
      ...
      WHERE;
        r.id = ?;
    `;
const rangeResult = await database_1.DB.query(fetchQuery, [rangeId]);
...
if (!session.user) {
    ...
    const currentResult = await database_1.DB.query();
                ...
                if (!session.user) {
                    ...
                    if (!session.user) {
                        ...
                        if (!session.user) {
                            ...
                            // Execute update;
                            await database_1.DB.query(updateQuery, updateParameters);
