...
const _GET = async (request) => {
    ...
    try {
        ...
        const doctorsResult = await DB.prepare(query).bind(...queryParams).all();
...
exports._GET = _GET;
const AddDoctorSchema = database_4.z.object({ user_id: database_4.z.number().int().positive("Valid User ID is required"),
    specialty: database_4.z.string().min(1, "Specialty is required"),
    qualifications: database_4.z.string().optional(),
    license_number: database_4.z.string().optional()
});
const _POST = async (request) => {
    ...
    try {
        ...
        const userCheck = await DB.prepare("SELECT role_id FROM Users WHERE user_id = ? AND is_active = TRUE").bind(doctorData.user_id).first();
        const doctorRole = await DB.prepare("SELECT role_id FROM Roles WHERE role_name = 'Doctor'").first();
        ...
        const insertResult = await DB.prepare("INSERT INTO Doctors (user_id, specialty, qualifications, license_number) VALUES (?, ?, ?, ?)").bind(doctorData.user_id, doctorData.specialty, doctorData.qualifications || null, doctorData.license_number || null).run();
