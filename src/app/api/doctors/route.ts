...
export const GET = async (request: Request) => {const cookieStore = await cookies();
  ...
  try {
    ...
    const doctorsResult = await DB.prepare(query).bind(...queryParams).all<Doctor & {full_name: string}>();
