...
if (!session.user) {
    ...
    const equipment = await database_1.biomedicalService.createBiomedicalEquipment(biomedicalData);
...
try { }
    ...
    return server_1.NextResponse.json();
    {
        error: "Failed to create biomedical equipment", details;
        error.message;
    }
    {
        status: 500;
...
try { }
    ...
    const regulatoryClass = searchParams.get("regulatoryClass") || undefined;
    const riskLevel = searchParams.get("riskLevel") || undefined;
    const calibrationDue = searchParams.get("calibrationDue") === "true";
    // Get biomedical equipment;
    const result = await database_1.biomedicalService.listBiomedicalEquipment({
        skip,
        take,
        search,
        equipmentType,
