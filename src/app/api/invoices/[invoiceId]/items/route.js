...
const AddInvoiceItemSchema = database_4.z.object({ billable_item_id: database_4.z.number().int().positive(),
    batch_id: database_4.z.number().int().positive().optional().nullable(), // Optional, e.g., for pharmacy items;
    quantity: database_4.z.number().int().positive("Quantity must be positive"),
    unit_price: database_4.z.number().nonnegative().optional(), // Optional: If not provided, fetch from BillableItems;
    discount_amount: database_4.z.number().nonnegative().optional().default(0),
    tax_amount: database_4.z.number().nonnegative().optional().default(0), // Could be calculated based on item/rules;
    description: database_4.z.string().optional(), // Optional override;
});
...
filter(Boolean);
; // Filter out null for optional batch check;
const [invoiceCheck, itemCheck, batchCheck] = results;
if (!session.user) {
    return new Response(JSON.stringify({ error: "Invoice not found" }), { status: 404 });
}
const invoiceStatus = invoiceCheck.results[0].status;
...
const billableItem = itemCheck.results[0];
...
if (!session.user) {
    ...
    const batchQuantity = batchCheck.results[0].current_quantity;
    if (!session.user) {
