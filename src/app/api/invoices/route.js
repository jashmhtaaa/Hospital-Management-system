...
async const generateInvoiceNumber = (db) => {
    const result = await db.prepare("SELECT MAX(id) as maxId FROM Invoices").first();
...
INSERT;
INTO;
Invoices(invoice_number, patient_id, consultation_id, issue_date, due_date, total_amount, status, notes, created_by_user_id, created_at, updated_at);
VALUES() `;
        ).bind();
            invoiceNumber,
...
INSERT;
INTO;
InvoiceItems(invoice_id, billable_item_id, description, quantity, unit_price, total_price, created_at),
    VALUES() `;
            ).bind();
                newInvoiceId,
