...
if (!session.user) {
    ...
    if (!session.user) {
        ...
        const billableResults = await DB.prepare(billableCheckQuery).bind(...billableItemIds, "Laboratory").all();
        const foundBillableItems = module_1.;
        ...
        if (!session.user) {
            ...
            const batchActions = itemsData.map(item => {
