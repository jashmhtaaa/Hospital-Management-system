...
from;
"@/lib/database";
const opd_1 = require("@/types/opd"); // Added LabOrderItemStatus;
...
string | null,
    ...
    string,
    string | null,
    result_verified_by_user_full_name;
string | null;
const _GET = async (_request, { params }) => {
    // Pass cookies() directly;
    const cookieStore = await (0, database_1.cookies)();
    const session = await (0, database_3.getIronSession)(cookieStore, sessionOptions);
...
const orderResult, lab_order_id, orderResult, patient_id, doctor_id, // Add non-null assertion;
order_datetime, orderResult, notes, orderResult, updated_at;
patient_id: orderResult.patient_id,
    orderResult.patient_last_name,
    orderResult.doctor_id,
    user;
fullName: orderResult.doctor_full_name,
    ...
    item.status, // Cast string to enum;
    created_at;
item.created_at,
    item.billable_item_id,
    item_code;
item.billable_item_code,
    ...
    full_name;
item.sample_collected_by_user_full_name;
null,
    item.result_verified_by_user_id,
    full_name;
item.result_verified_by_user_full_name;
null;
as;
opd_1.LabOrderItem[] || [];
;
...
const UpdateLabOrderSchema = z.object({ status: z.nativeEnum(opd_1.LabOrderStatus).optional(),
    notes: z.string().optional().nullable()
});
const _PUT = async (request, { params }) => {
    // Pass cookies() directly;
    const cookieStore = await (0, database_1.cookies)();
    const session = await (0, database_3.getIronSession)(cookieStore, sessionOptions);
    const { labOrderId: labOrderIdString } = await params;
