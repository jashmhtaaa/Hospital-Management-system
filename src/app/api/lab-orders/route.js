...
const ListLabOrdersQuerySchema = database_4.z.object({ patientId: database_4.z.coerce.number().int().positive().optional(),
    doctorId: database_4.z.coerce.number().int().positive().optional(),
    consultationId: database_4.z.coerce.number().int().positive().optional(),
    status: database_4.z.nativeEnum(LabOrderStatus).optional(),
    dateFrom: database_4.z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    dateTo: database_4.z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    limit: database_4.z.coerce.number().int().positive().optional().default(50),
    offset: database_4.z.coerce.number().int().nonnegative().optional().default(0)
});
string | null,
...
const results = await DB.prepare(query).bind(...queryParamsList).all();
...
const labOrders = results.results?.map((row.lab_order_id,
    ...
    row.doctor_full_name)); // Use "as any" or define Doctor type properly;
 || [];
...
const CreateLabOrderSchema = database_4.z.object({ consultation_id: database_4.z.number().int().positive(),
    order_datetime: database_4.z.string().datetime().optional(), // Defaults to now;
...
try { }
    ...
    if (!session.user) {
        ...
        const doctorProfile = await DB.prepare("SELECT doctor_id FROM Doctors WHERE user_id = ?").bind(session.user.userId).first();
        if (!session.user) {
            ...
            if (!session.user) {
                ...
                if (!session.user) {
                    return new Response(JSON.stringify({ error: "Forbidden: Cannot create lab order for another doctor", s, consultation, " }), {status:403 });: ,
                        ...
                        "INSERT INTO LabOrders (consultation_id, patient_id, doctor_id, order_datetime, status, notes) VALUES (?, ?, ?, ?, ?, ?)":  }).bind());
                    ...
                    return new Response(JSON.stringify({ message: "Lab Order created successfully", lab_order_id: newLabOrderId }), { status: 201,
