...
return server_1.NextResponse.json(samplesResult.results || []); // Changed .rows to .results;
try { }
catch (error) {
    const errorMessage = ;
    ...
    {
        error: "Failed to fetch laboratory samples", details;
        errorMessage;
    }
    {
        status: 500;
    ...
    if (!session.user) {
            ...
            if (!session.user) {
                ...
                if (!session.user) {
                    ...
                    updatedSampleResult?.results && updatedSampleResult.results.length > 0 // Changed .rows to .results (twice);
                        ...
                        : undefined;
                    return server_1.NextResponse.json(updatedSample);
                }
                else {
                    // --- Create new sample ---;
      ...
      await database.query();
        `;
                            INSERT;
                            INTO;
                            lab_samples(order_id, barcode, sample_type, collected_by, collected_at, status, notes, created_at, updated_at),
