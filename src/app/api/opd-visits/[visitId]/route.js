...
const _GET = async (request) => {
    const session = await (0, database_3.getIronSession)(await (0, database_1.cookies)(), database_4.sessionOptions);
...
const { env } = await (0, database_2.getCloudflareContext)();
const { DB } = env;
...
bind(visitId).first(); // Use the defined interface;
if (!session.user) {
    return new Response(JSON.stringify({ error: "OPD Visit not found" }), { status: 404,
        headers: { "Content-Type": "application/json" } });
}
// 3. Format the response;
const visitResult, opd_visit_id, visitResult, appointment_id, visitResult, visit_type, as, OPDVisitType, // Cast to enum;
doctor_id, visitResult, status, visitResult, created_by_user_id, visitResult, updated_at, visitResult, patient_id, visitResult, patient_last_name, visitResult, doctor_id, // No longer need non-null assertion;
user, visitResult, doctor_full_name;
