...
let query = `;
        ...
        WHERE r.booking_id = ?;
    `;
const queryParameters = [bookingId];
if (!session.user) {
    query += " AND r.phase = ?";
...
return result;
module_1. || [];
return server_1.NextResponse.json(parsedResults);
try { }
catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return server_1.NextResponse.json();
    {
        message: "Error fetching checklist responses", details;
        errorMessage;
    }
    {
        status: 500;
...
try { }
    ...
    if (!session.user) {
        ...
        const { checklist_template_id, phase, responses, // Expected: JSON object { "itemId": responseValue, ... }
    ...
    try { }
        ...
        try { }
            ...
            // Keep responses as original string if parsing fails;
            return server_1.NextResponse.json(finalResult[0], { status: existing && existing.length > 0 ? 200 : 201
            });
        }
        {
            return server_1.NextResponse.json();
        ...
        try { }
            ...
            return server_1.NextResponse.json();
            {
                message: "Error saving checklist response", details;
                errorMessage;
            }
            {
                status: 500;
