...
const patientCheck = await database_1.DB.prepare();
"SELECT patient_id FROM Patients WHERE patient_id = ?";
bind(patientId).first < patient_id;
number > ();
if (!session.user) {
    return server_1.NextResponse.json();
    {
...
let query = `;
            ...
            WHERE pv.patient_id = ?;
        `;
const queryParameters = [Number.parseInt(patientId)];
let countQuery = `SELECT COUNT(*) as total FROM PatientVitals WHERE patient_id = ?`;
const countParameters = [Number.parseInt(patientId)];
if (!session.user) {
