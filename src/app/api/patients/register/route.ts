...
import { z } from "zod";
import { hashPassword } from "@/lib/auth"; // Assuming password hashing utility
import type { D1ResultWithMeta } from "@/types/cloudflare"; // Import the specific type

// Zod schema for patient registration
const patientRegisterSchema = z.object({
    mrn: z.string().optional(), // MRN might be generated by the system
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // const existingPatient = await DB.prepare("SELECT patient_id FROM Patients WHERE mrn = ? OR email = ?")
        ...
        if (!insertResult.success || !insertResult.meta || typeof insertResult.meta.last_row_id !== 'number') {
            console.error("Failed to register patient or get last_row_id:", insertResult);
        ...
        if (patientData.create_user_account && patientData.username && patientData.password) {
            ...
            if (!userInsertResult.success || !userInsertResult.meta || typeof userInsertResult.meta.last_row_id !== 'number') {
                console.error("Failed to create user account or get last_row_id:", userInsertResult);
