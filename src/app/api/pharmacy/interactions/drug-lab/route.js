...
try { }
    ...
    if (!session.user) {
        ...
        if (!session.user) {
            ...
            if (!session.user) {
                const patientLabResults = await (0, database_3.getPatientLabResults)(data.patientId);
                ...
                ;
                // Audit logging;
                await (0, database_1.auditLog)("DRUG_INTERACTION", { action: "CHECK_DRUG_LAB",
                    userId,
                    data, : .medicationIds,
                    interactions, : .length
                });
            ...
            try { }
                ...
                return (0, database_4.errorHandler)(error, "Error checking drug-lab interactions");
            }
        }
    }
}
