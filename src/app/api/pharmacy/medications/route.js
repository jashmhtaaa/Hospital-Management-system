...
"@/lib/database";
from;
"@/lib/database"; // Assuming db returns a promise;
...
searchParams.has("prescription_required");
searchParams.get("prescription_required") === "true";
undefined,
    narcotic;
searchParams.has("narcotic");
...
try { }
    ...
    {
        error: "Failed to fetch medications", details;
        message;
    }
    {
        status: 500;
    ...
    if (!session.user) {
      ...
      RETURNING id`; // Use RETURNING to get the new ID;
        ...
        if (!session.user) {
            ...
            bind(newId);
            first();
            return server_1.NextResponse.json(newMedication, { status: 201 });
        }
        try { }
        catch (error) {
            const message = ;
                    ...
                    message.includes("item_code");
            {
                ...
                return server_1.NextResponse.json();
                {
                    error: "Failed to create medication", details;
                    message;
                }
                {
                    status: 500;
