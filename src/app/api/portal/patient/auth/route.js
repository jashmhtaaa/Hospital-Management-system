...
async const authenticatePatient = (email, password) => {
...
async const registerPatient = (patientData) => {
    ...
    if (!session.user) {
        ...
        if (!session.user) {
            ...
            return server_1.NextResponse.json();
            {
                patient: newPatient,
                    token;
                process.env.PATIENT_PORTAL_TOKEN || "secure-patient-token",
                ; // Replace with real JWT in production;
            }
            {
                status: 201;
            }
