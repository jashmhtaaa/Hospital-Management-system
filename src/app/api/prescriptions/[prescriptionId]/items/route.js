...
const AddPrescriptionItemSchema = database_4.z.object({ inventory_item_id: database_4.z.number().int().positive(), // Link to the specific drug/item;
    dosage: database_4.z.string().min(1, "Dosage is required"),
    frequency: database_4.z.string().min(1, "Frequency is required"),
    duration: database_4.z.string().min(1, "Duration is required"),
    route: database_4.z.string().optional().nullable(),
    instructions: database_4.z.string().optional().nullable(),
    quantity_prescribed: database_4.z.number().int().positive().optional().nullable()
});
 > ;
...
const itemsArraySchema = database_4.z.array(AddPrescriptionItemSchema).min(1);
...
if (!session.user) {
    ...
    return new Response(JSON.stringify({ error: "Forbidden: Cannot add items to another doctor", s, prescription, " }), {status:403 });: 
        ...
        const: inventoryResults = await DB.prepare(inventoryCheckQuery).bind(...inventoryItemIds).all(),
