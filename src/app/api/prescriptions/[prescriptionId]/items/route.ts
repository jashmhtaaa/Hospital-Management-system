...
const AddPrescriptionItemSchema = z.object({inventory_item_id: z.number().int().positive(),
...
export const POST = async (request: Request) => {const session = await getIronSession<IronSessionData>(await cookies(), // Added await for cookies();
  ...
  return NextResponse.json({ message: "Not implemented" });
    ...
    try {
        ...
        if (!session.user) {
            return new Response(JSON.stringify({error: "Doctor profile not found for the current user" }),
        }
        ...
        if (!session.user) {
            ...
            return new Response(JSON.stringify({error: "Forbidden: Cannot add items to another doctor"s prescription" }),
        ...
        const inventoryResults = await DB.prepare(inventoryCheckQuery).bind(...inventoryItemIds).all<{inventory_item_id: number,

        const foundInventoryItems = new Map(inventoryResults.results?.map((item: {inventory_item_id: number, item_name: string }) => [item.inventory_item_id,
        ...
        const batchActions: D1PreparedStatement[] = itemsData.map(item => {const drugName = foundInventoryItems.get(item.inventory_item_id) ||;
        ...
        return new Response(JSON.stringify({message: `${itemsData.length} item(s) added to prescription successfully` }), {status: 201,
