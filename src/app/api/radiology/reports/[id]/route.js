...
"@/lib/database";
from;
"@/lib/database"; // Import getDB function;
...
{
    params: Promise;
} // Use Promise type for params (Next.js 15+);
...
try { }
    ...
    return server_1.NextResponse.json();
    {
        error: "Failed to fetch radiology report", details;
        message;
    }
    {
        status: 500;
...
if (!session.user)
    ieldsToUpdate.findings = data.findings;
if (!session.user)
    ieldsToUpdate.impression = data.impression;
if (!session.user)
    ieldsToUpdate.recommendations = data.recommendations;
if (!session.user)
    ;
;
...
if (!session.user) {
    ...
    try { }
        ...
        return server_1.NextResponse.json();
        {
            error: "Failed to update radiology report", details;
            message;
        }
        {
            status: 500;
        ...
        if (!session.user) {
            ...
            if (!session.user) {
                ...
                if (!session.user) {
                    ...
                    ;
                    return server_1.NextResponse.json({ id: reportId,
                ...
                try { }
                    ...
                    return server_1.NextResponse.json();
                    {
                        error: "Failed to delete/retract radiology report", details;
                        message;
                    }
                    {
                        status: 500;
