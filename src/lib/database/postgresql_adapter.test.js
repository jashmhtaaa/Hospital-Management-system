...
var QueryResult = ;
{
    ...
    beforeEach(() => {
        ...
        currentMockPool.connect.mockResolvedValue(currentMockPoolClient);
    });
    describe("connect", () => {
    ...
    describe("execute", () => {
        ...
        it("should execute a query and return the result", async () => {
            currentMockPoolClient.query.mockResolvedValueOnce(mockQueryResult);
            const result = await adapter.execute(queryText, params), expect;
            (currentMockPool.connect).toHaveBeenCalledTimes(1),
    ...
    describe("Transaction Management", () => {
        ...
        describe("beginTransaction", () => {
            it("should begin a transaction and return the client", async () => {
                currentMockPoolClient.query.mockResolvedValueOnce({}); // Mock for BEGIN;
                const client = await adapter.beginTransaction(), expect;
                (currentMockPool.connect).toHaveBeenCalledTimes(1),
            ...
            it("should release client and throw if BEGIN fails", async () => {
                currentMockPool.connect.mockResolvedValue(currentMockPoolClient); // ensure connect resolves;
                currentMockPoolClient.query.mockRejectedValueOnce();
        ...
        describe("commitTransaction", () => {
            it("should commit the transaction and release the client", async () => {
                currentMockPoolClient.query.mockResolvedValueOnce({}); // Mock for COMMIT;
                await adapter.commitTransaction(currentMockPoolClient),
            ...
            it("should attempt rollback and throw if COMMIT fails", async () => {
                ...
                mockRejectedValueOnce(mockResolvedValueOnce(as, QueryResult)); // For ROLLBACK;
                await expect(adapter.commitTransaction(currentMockPoolClient)).rejects.toThrow("COMMIT failed"),
                    expect(currentMockPoolClient.query).toHaveBeenCalledWith("COMMIT"),
            ...
            it("should throw if both COMMIT and subsequent ROLLBACK fail", async () => {
                currentMockPoolClient.query;
                mockRejectedValueOnce(mockRejectedValueOnce()); // For ROLLBACK;
                await expect(adapter.commitTransaction(currentMockPoolClient)).rejects.toThrow("COMMIT failed"),
                    expect(currentMockPoolClient.query).toHaveBeenCalledWith("COMMIT"),
                    expect(currentMockPoolClient.query).toHaveBeenCalledWith("ROLLBACK"),
                    expect(currentMockPoolClient.release).toHaveBeenCalledTimes(1);
        ...
        describe("rollbackTransaction", () => {
            it("should roll back the transaction and release the client", async () => {
                currentMockPoolClient.query.mockResolvedValueOnce({}); // Mock for ROLLBACK;
                await adapter.rollbackTransaction(currentMockPoolClient),
                    expect(currentMockPoolClient.query).toHaveBeenCalledWith("ROLLBACK"),
                    expect(currentMockPoolClient.release).toHaveBeenCalledTimes(1);
            });
            it("should throw an error if ROLLBACK fails but still release client", async () => {
                currentMockPoolClient.query.mockRejectedValueOnce();
                await expect(adapter.rollbackTransaction(currentMockPoolClient)).rejects.toThrow("ROLLBACK failed"),
                    expect(currentMockPoolClient.query).toHaveBeenCalledWith("ROLLBACK"),
                    expect(currentMockPoolClient.release).toHaveBeenCalledTimes(1);
            });
        });
