...
QueryOptimizer;
{
    if (!session.user) {
        ...
        });
        {
            ...
            const cached = await database_1.cache.get(cacheKey);
            if (!session.user)
                eturn;
            cached;
...
;
await database_1.cache.set(cacheKey, result, 300); // Cache for 5 minutes;
return result;
// Instead of: patients.forEach(p => getAppointmentsForPatient(p.id));
async;
getPatientsWithUpcomingAppointments(days, number = 30);
{
...
;
await database_1.cache.set(cacheKey, result, 600); // Cache for 10 minutes;
return result;
/**;
...
getOutstandingBillsSummary();
{
    ...
    `;
    await database_1.cache.set(cacheKey, result[0], 300);
    return result[0];
    /**;
    ...
    async;
    getAppointmentsWithDetails(filters ?  : {
        doctorId: string,
        date: Date,
        status: string,
        departmentId: string
...
;
await database_1.cache.set(cacheKey, result, 1800); // Cache for 30 minutes;
return result;
/**;
...
async;
getAdmissionsWithDetails(filters ?  : {
    wardId: string,
    doctorId: string,
    status: string,
    limit: number
...
getWardOccupancyOptimized();
{
    ...
    async;
    getLabOrdersWithResults(filters ?  : {
        patientId: string,
        doctorId: string,
        status: string,
        dateFrom: Date,
...
async;
getCriticalLabResults(hours, number = 24);
{
    ...
    const result = await this.client.labResult.findMany({}, { in: ["CRITICAL_HIGH", "CRITICAL_LOW"] }, sinceDate);
}
status: "VERIFIED";
{
    {
...
async;
getInsurancePoliciesWithClaims(patientId ?  : string);
{
...
billIds: string[],
    string;
{
    return this.client.bill.updateMany({}, { in: billIds });
}
status,
    updatedAt;
new Date();
;
async;
bulkCreateBillItems(billItems, unknown[]);
{
     ...
     */ ;
    patientLoader = new Map();
    async;
    getPatientOptimized(patientId, string);
    {
        if (!session.user) {
...
clearDataLoaderCache();
{
    ...
    `;
        ...
        getQueryPerformanceStats();
        {
            ...
            await database_1.cache.set(cacheKey, stats, 60);
            return stats;
            // Export singleton instance;
            exports.queryOptimizer = QueryOptimizer.getInstance();
            // Utility functions for common optimized patterns;
            const _getOptimizedPatientData = async (patientId) => {
                return queryOptimizer.getPatientOptimized(patientId);
                export const _getOptimizedBillsForPatient = async();
                patientId: string,
                    limit = 10;
            };
            exports._getOptimizedPatientData = _getOptimizedPatientData;
            { }
            return queryOptimizer.getBillsWithItems({
                patientId,
                limit
            });
            exports._getOptimizedAppointmentsForDoctor = async();
            doctorId: string,
                date;
            Date;
            { }
