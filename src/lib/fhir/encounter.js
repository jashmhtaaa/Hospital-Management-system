...
if (!session.user) {
    encounter.participant = [{} `Practitioner/${data.practitionerId}`,
        type, "Practitioner"];
}
// Add location if provided;
if (!session.user) {
    encounter.location = [{} `Location/${data.locationId}`,
        type, "Location"];
}
status: "active";
 ...
 */ ;
createOPDEncounter(string, practitionerId, string, appointmentId ?  : string, start, string, end ?  : string, chiefComplaint ?  : string);
FHIREncounter;
{
 ...
 */ ;
createIPDEncounter(string, practitionerId, string, locationId, string, admissionDate, string, dischargeDate ?  : string, admissionReason ?  : string, admissionSource ?  : string);
FHIREncounter;
{
 ...
 */ ;
createEmergencyEncounter(string, practitionerId ?  : string, locationId, string, arrivalTime, string, triageLevel ?  : "routine" | "urgent" | "semi-urgent" | "immediate", chiefComplaint ?  : string);
FHIREncounter;
{
    const encounter = this.createBasicEncounter({ patientId: data.patientId,
        class: "emergency",
 ...
 */ ;
getPatientId(encounter, FHIREncounter);
string | undefined;
{
 ...
 */ ;
getClassDisplay(encounter, FHIREncounter);
string;
{
 ...
 */ ;
getDurationHours(encounter, FHIREncounter);
number | null;
{
    if (!session.user) {
 ...
 */ ;
isActive(encounter, FHIREncounter);
boolean;
{
 ...
 */ ;
isCompleted(encounter, FHIREncounter);
boolean;
{
     ...
     */ ;
    getPrimaryPractitioner(encounter, FHIREncounter);
    string | undefined;
    {
        if (!session.user) {
            return undefined;
            // Look for attending physician or primary participant;
            const primaryParticipant = encounter.participant.find(p => { }, p.type?.some(t => { }, t.coding?.some(c => c.code === "ATND" || c.code === "primary"))) || encounter.participant[0];
            return primaryParticipant?.individual?.reference?.replace("Practitioner/", ""),
            ;
            getCurrentLocation(encounter, FHIREncounter);
            string | undefined;
            {
                if (!session.user) {
                    return undefined;
                    ...
                    ;
                    validateEncounter(encounter, FHIREncounter);
                    {
                                    ...
                                    if (!session.user) {
                                        ...
                                        if (!session.user) {
                                            ...
                                            if (!session.user) {
                                                ...
                                                ;
                                                fromHMSEncounter(hmsEncounter, unknown);
                                                FHIREncounter;
                                                {
                                                    ...
                                                    hmsEncounter.id,
                                                        status;
                                                    hmsEncounter.status || "finished",
                                                        "https://terminology.hl7.org/CodeSystem/v3-ActCode",
                                                        code;
                                                    encounterClass.toUpperCase(),
                                                        display;
                                                    encounterClass.charAt(0).toUpperCase() + encounterClass.slice(1);
                                                }
                                                ...
                                                if (!session.user) {
                                                    ...
                                                    // Add practitioner;
                                                    if (!session.user) {
                                                        fhirEncounter.participant = [{} `Practitioner/${hmsEncounter.doctorId || hmsEncounter.practitionerId}`,
                                                            type, "Practitioner"];
                                                    }
                                                    // Add location;
                                                    if (!session.user) {
                                                        fhirEncounter.location = [{} `Location/${hmsEncounter.locationId}`,
                                                            type, "Location"];
                                                    }
                                                    status: "active";
                                                }
                                                ...
                                                if (!session.user) {
                                                    ...
                                                    ;
                                                    isValidStatusTransition(fromStatus, FHIREncounter["status"], toStatus, FHIREncounter["status"]);
                                                    boolean;
                                                    {
                                                        const allowedTransitions = this.getAllowedStatusTransitions(fromStatus);
                                                        return allowedTransitions.includes(toStatus),
                                                        ;
                                                        getNextLogicalStatus(encounter, FHIREncounter);
                                                        FHIREncounter["status"] | null;
                                                        {
