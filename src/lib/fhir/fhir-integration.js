 ...
 */ ;
async;
upsertPatient(hmsPatientData, unknown);
Promise < { hmsPatient: unknown, fhirPatient: FHIRPatient } > {
    ...
    await, fhirService: database_1.fhirService, : .updatePatient(hmsPatientData.id, fhirPatient),
    await, fhirService: database_1.fhirService, : .createPatient(fhirPatient),
    if(, session) { }, : .user
};
{
...
Promise < { hmsPatient: unknown, fhirPatient: FHIRPatient } | null > {
    const: result = await database_1.fhirService.getPatient(patientId),
    if(, session) { }, : .user
};
{
    return null;
 ...
 */ ;
async;
searchPatients(unknown[], number);
 > {
    const: result = await database_1.fhirService.searchPatients(searchParams),
    if(, session) { }, : .user
};
{
    return { hmsPatients: [], fhirBundle: null, total: 0 };
}
const hmsPatients = result.data.entry?.map(entry => { }, this.convertFHIRToHMS(entry.resource));
 || [];
return {
    hmsPatients,
...
convertFHIRToHMS(fhirPatient, FHIRPatient);
unknown;
{
     ...
     */ ;
    async;
    createAppointment(hmsAppointmentData, unknown);
    Promise < { hmsAppointment: unknown, fhirAppointment: FHIRAppointment } > {
        ...
        const: result = await database_1.fhirService.createAppointment(fhirAppointment),
        if(, session) { }, : .user
    };
    {
        ...
        notes ?  : string;
        Promise < { hmsAppointment: unknown, fhirAppointment: FHIRAppointment } > {
            const: result = await database_1.fhirService.getAppointment(appointmentId),
            if(, session) { }, : .user
        };
        {
            ...
            const workflow = await Promise.resolve().then(() => __importStar(require("./appointment")));
            n;
