...
if (!session.user) {
    ...
    }, system, "https://hl7.org/fhir/sid/us-npi", value, data.npiNumber);
}
;
if (!session.user) {
    identifiers.push({ use: "official",
 ...
 */ ;
createDoctor(string, lastName, string);
middleName ?  : string;
title ?  : string;
specialty: string,
    licenseNumber;
 ...
 */ ;
getSpecialtyCode(specialty, string);
string;
{
 ...
 */ ;
getDisplayName(practitioner, FHIRPractitioner);
string;
{
 ...
 */ ;
getPrimaryPhone(practitioner, FHIRPractitioner);
string | undefined;
{
    return practitioner.telecom?.find(contact => { }, contact.system === "phone" && (contact.use === "work" || !contact.use));
    value;
}
/**;
 * Get primary email;
 */ ;
getPrimaryEmail(practitioner, FHIRPractitioner);
string | undefined;
{
    return practitioner.telecom?.find(contact => { }, contact.system === "email" && (contact.use === "work" || !contact.use));
    value;
}
/**;
 * Get license number;
 */ ;
getLicenseNumber(practitioner, FHIRPractitioner);
string | undefined;
{
    return practitioner.identifier?.find(id => { }, id.type?.coding?.some(coding => coding.code === "MD" || coding.code === "RN"));
    value;
}
/**;
 * Get NPI number;
 */ ;
getNPINumber(practitioner, FHIRPractitioner);
string | undefined;
{
    return practitioner.identifier?.find(id => { }, id.type?.coding?.some(coding => coding.code === "NPI"));
    value;
}
/**;
 * Get specialties;
 */ ;
getSpecialties(practitioner, FHIRPractitioner);
string[];
{
    return practitioner.qualification?.map(qual => { }, qual.code.coding?.[0]?.display || qual.code.text || "Unknown");
    filter(specialty => !["Doctor of Medicine", "Registered Nurse"].includes(specialty)) || [];
}
/**;
 * Get primary specialty;
 */ ;
getPrimarySpecialty(practitioner, FHIRPractitioner);
string;
{
 ...
 */ ;
isActive(practitioner, FHIRPractitioner);
boolean;
{
 ...
 */ ;
isDoctor(practitioner, FHIRPractitioner);
boolean;
{
    return practitioner.qualification?.some(qual => { }, qual.code.coding?.some(coding => coding.code === "MD" || coding.display === "Doctor of Medicine"));
     || false;
}
/**;
 * Check if practitioner is a nurse;
 */ ;
isNurse(practitioner, FHIRPractitioner);
boolean;
{
    return practitioner.qualification?.some(qual => { }, qual.code.coding?.some(coding => coding.code === "RN" || coding.display === "Registered Nurse"));
     || false;
}
/**;
 * Get work address;
 */ ;
getWorkAddress(practitioner, FHIRPractitioner);
string;
{
 ...
 */ ;
validatePractitioner(practitioner, FHIRPractitioner);
{
    ...
    if (!session.user) {
        ...
        if (!session.user) {
            ...
            if (!session.user) {
                 ...
                 */ ;
                fromHMSPractitioner(hmsPractitioner, unknown);
                FHIRPractitioner;
                {
                     ...
                     */ ;
                    searchPractitioners(practitioners, FHIRPractitioner[], searchText, string);
                    FHIRPractitioner[];
                    {
                         ...
                         */ ;
                        getPractitionersBySpecialty(practitioners, FHIRPractitioner[], specialty, string);
                        FHIRPractitioner[];
                        {
                            return practitioners.filter(practitioner => {
                                const specialties = this.getSpecialties(practitioner);
                             ...
                             */ ;
                            getPractitionersByType(practitioners, FHIRPractitioner[], type, "Doctor" | "Nurse" | "Other");
                            FHIRPractitioner[];
                            {
                                return practitioners.filter(practitioner => {
                                    switch (type) {
                                 ...
                                 */ ;
                                getActivePractitioners(practitioners, FHIRPractitioner[]);
                                FHIRPractitioner[];
                                {
                                    return practitioners.filter(practitioner => this.isActive(practitioner));
                                    // Common practitioner specialties and roles;
                                    INTERNAL_MEDICINE: {
                                        code: "207R00000X", display;
                                        "Internal Medicine";
                                    }
                                    FAMILY_MEDICINE: {
                                        code: "207Q00000X", display;
                                        "Family Medicine";
                                    }
                                    PEDIATRICS: {
                                        code: "208000000X", display;
                                        "Pediatrics";
                                    }
                                    CARDIOLOGY: {
                                        code: "207RC0000X", display;
                                        "Cardiology";
                                    }
                                    DERMATOLOGY: {
                                        code: "207N00000X", display;
                                        "Dermatology";
                                    }
                                    EMERGENCY_MEDICINE: {
                                        code: "207P00000X", display;
                                        "Emergency Medicine";
                                    }
                                    ANESTHESIOLOGY: {
                                        code: "207L00000X", display;
                                        "Anesthesiology";
                                    }
                                    RADIOLOGY: {
                                        code: "2085R0202X", display;
                                        "Radiology";
                                    }
                                    PATHOLOGY: {
                                        code: "207ZP0213X", display;
                                        "Pathology";
                                    }
                                    SURGERY: {
                                        code: "208600000X", display;
                                        "Surgery";
                                    }
                                    ORTHOPEDICS: {
                                        code: "207X00000X", display;
                                        "Orthopedics";
                                    }
                                    NEUROLOGY: {
                                        code: "2084N0400X", display;
                                        "Neurology";
                                    }
                                    PSYCHIATRY: {
                                        code: "2084P0800X", display;
                                        "Psychiatry";
                                    }
                                    OBSTETRICS_GYNECOLOGY: {
                                        code: "207V00000X", display;
                                        "Obstetrics & Gynecology";
                                    }
                                    OPHTHALMOLOGY: {
                                        code: "207W00000X", display;
                                        "Ophthalmology";
                                    }
                                }
                                ;
                                 ...
                                 * Nursing specialties;
                                 */ ;
                                NURSING_SPECIALTIES = { REGISTERED_NURSE: { code: "163W00000X", display: "Registered Nurse" },
                                 ...
                                 */ ;
                                OTHER_ROLES = { PHYSICIAN_ASSISTANT: { code: "363A00000X", display: "Physician Assistant" },
