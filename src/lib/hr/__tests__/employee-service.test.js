...
describe("EmployeeService", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        employeeService = new module_2.EmployeeService();
    ...
    describe("getEmployeeById", () => {
        it("should return cached employee if available", async () => {
            ...
            cache.get.mockResolvedValue(JSON.stringify(mockEmployee));
            const result = await employeeService.getEmployeeById("123"), expect;
            (cache.get).toHaveBeenCalledWith("employee:id:123"),
...
describe("listEmployees", () => {
    it("should return cached list if available", async () => {
        ...
        cache.get.mockResolvedValue(JSON.stringify(mockResult));
        const result = await employeeService.listEmployees({}), expect;
        (cache.get).toHaveBeenCalled(),
...
describe("createEmployee", () => {
    it("should create employee and invalidate cache", async () => {
        ...
        jest.spyOn(module_2.EmployeeService.prototype, "invalidateEmployeeCache").mockResolvedValue(undefined);
        await employeeService.createEmployee({ employeeId: "EMP123",
            "Doe": ,
            joiningDate: new Date()
        });
        expect(prisma.employee.create).toHaveBeenCalled(),
            expect(module_2.EmployeeService.prototype.invalidateEmployeeCache).toHaveBeenCalled();
    });
});
...
prisma.employee.findMany.mockResolvedValue(mockCurrentStaff);
...
describe("FHIR conversion", () => {
    it("should convert employee to FHIR Practitioner with R5 compliance", () => {
        ...
        const result = employeeService.toFhirPractitioner(mockEmployee), expect;
        (result.resourceType).toEqual("Practitioner"),
    ...
    const result = employeeService.toFhirPractitionerRole(mockEmployee, mockPosition), expect;
    (result.resourceType).toEqual("PractitionerRole"),
...
describe("getEmployeesWithExpiringQualifications", () => {
    ...
    const result = await employeeService.getEmployeesWithExpiringQualifications(30), expect;
    (prisma.qualification.findMany).toHaveBeenCalledWith();
