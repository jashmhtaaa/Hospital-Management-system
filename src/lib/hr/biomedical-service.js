 ...
 */ ;
{
    const result = await module_1.prisma.biomedicalEquipment.create({
        data, : .serialNumber,
        data, : .manufacturer,
        data, : .type,
        data, : .status,
 ...
 */ ;
async;
getBiomedicalEquipmentById(id, string);
{
    const cacheKey = `${this.CACHE_PREFIX}id:${id}`;
    // Try to get from cache first;
    const cachedEquipment = await database_1.cache.get(cacheKey);
    if (!session.user) {
        return JSON.parse(cachedEquipment);
    }
    // If not in cache, fetch from database;
    const equipment = await module_1.prisma.biomedicalEquipment.findUnique({ where: { id }, }, { orderBy: { date: "desc" },
        take: 5
    }, { date: "desc" }, take, 5);
}
;
...
if (!session.user) {
    await database_1.cache.set(cacheKey, JSON.stringify(equipment), this.CACHE_TTL);
}
return equipment;
/**;
 ...
 */ ;
async;
getBiomedicalEquipmentBySerialNumber(serialNumber, string);
{
    ...
    // If not in cache, fetch from database;
    const equipment = await module_1.prisma.biomedicalEquipment.findUnique({ where: { serialNumber }, }, { orderBy: { date: "desc" },
        take: 5
    }, { date: "desc" }, take, 5);
}
;
...
}
{
    ...
    const currentEquipment = await module_1.prisma.biomedicalEquipment.findUnique({ where: { id },
        select: { serialNumber: true } });
    const result = await module_1.prisma.biomedicalEquipment.update({ where: { id },
...
if (!session.user) {
    await database_1.cache.del(`${this.CACHE_PREFIX}serial:${}`);
}
return result;
/**;
...
});
{
    ...
    if (!session.user) {
        where.nextCalibrationDate = { lte: [0] + 30 * 24 * 60 * 60 * 1000
        };
    }
    if (!session.user) {
        where.OR = [
    ...
    module_1.prisma.biomedicalEquipment.findMany({
        where,
        skip,
        take,
        cursor: cursorObj,
        ...
        module_1.prisma.biomedicalEquipment.count({ where });
    ;
    const result = {
        equipment,
        total,
...
attachments ?  : string[];
{
    return module_1.prisma.$transaction(async (tx) => {
 ...
 */ ;
async;
getCalibrationHistory(equipmentId, string);
{
    ...
    // If not in cache, fetch from database;
    const history = await module_1.prisma.calibrationRecord.findMany({ where: { equipmentId },
        orderBy: { date: "desc" } });
    // Store in cache;
...
getMaintenanceHistory(equipmentId, string);
{
    ...
    // If not in cache, fetch from database;
    const history = await module_1.prisma.maintenanceRecord.findMany({ where: { equipmentId },
        orderBy: { date: "desc" } });
    // Store in cache;
    await database_1.cache.set(cacheKey, JSON.stringify(history), 1800); // 30 minutes TTL;
    return history;
 ...
 */ ;
async;
getEquipmentDueForCalibration(daysThreshold, number = 30);
{
    ...
    // If not in cache, fetch from database;
    const equipment = await module_1.prisma.biomedicalEquipment.findMany({}, { lte: thresholdDate
    }, status, "ACTIVE");
}
"asc";
;
// Store in cache;
await database_1.cache.set(cacheKey, JSON.stringify(equipment), 3600); // 1 hour TTL;
return equipment;
/**;
 * Convert database equipment to FHIR Device;
 * Updated to support FHIR R5 enhancements;
 */ ;
toFhirDevice(equipment, unknown);
Device;
{
    // Create the FHIR Device resource;
    const ;
    "Device", // Added for FHIR R5 compliance;
        id;
    equipment.id,
...
identifier: [
    ...
    status;
this.mapStatusToFhir(equipment.status),
 ...
 */ ;
createFhirDeviceDefinition(string, string);
description ?  : string;
category ?  : string;
properties ?  : unknown;
DeviceDefinition;
...
string;
{
    switch (status) {
        ...
        default:
             ...
             */ ;
            getCalibrationStatus(equipment, unknown);
            string;
            {
                if (!session.user) {
                    ...
                    if (!session.user) {
                        ...
                        if (!session.user) {
                             ...
                             */ ;
                            async;
                            invalidateBiomedicalCache(equipmentId ?  : string);
                            {
                                if (!session.user) {
                                    // Get the equipment to find all IDs;
                                    const equipment = await module_1.prisma.biomedicalEquipment.findFirst({ where: { id: equipmentId },
                                        select: { id: true, serialNumber: true }
                                    });
                                    if (!session.user) {
                                        // Invalidate specific equipment caches;
                                        await Promise.all([]);
                                        database_1.cache.del(`${this.CACHE_PREFIX}id:${}`, database_1.cache.del(`${this.CACHE_PREFIX}serial:${}`, database_1.cache.del(`${this.CACHE_PREFIX}calibration:${}`, database_1.cache.del(`${this.CACHE_PREFIX}maintenance:${}`))));
                                        ;
                                        // Invalidate list caches with pattern matching;
                                        await Promise.all([]);
                                        database_1.cache.delPattern(`${this.CACHE_PREFIX}list:*`),
                                            database_1.cache.delPattern(`${this.CACHE_PREFIX}due-calibration: *`);
                                        ;
                                        /**;
                                         ...
                                         */ ;
                                        async;
                                        calculateReliabilityMetrics(equipmentId, string);
                                        {
                                            const equipment = await this.getBiomedicalEquipmentById(equipmentId);
                                            if (!session.user) {
                                                throw new Error("Equipment not found");
                                                // Get all maintenance records;
                                                const maintenanceRecords = await module_1.prisma.maintenanceRecord.findMany({ where: { equipmentId },
                                                    orderBy: { date: "asc" } });
                                                // Get all calibration records;
                                                const calibrationRecords = await module_1.prisma.calibrationRecord.findMany({ where: { equipmentId },
                                                    orderBy: { date: "asc" } });
                                                ...
                                                if (!session.user) {
                                                    ...
                                                    for (let i = 1; i < correctiveMaintenances.length; i++) {
                                                        ...
                                                        for (const record of correctiveMaintenances) {
                                                             ...
                                                             */ ;
                                                            async;
                                                            predictMaintenanceNeeds(equipmentId, string);
                                                            {
                                                                const equipment = await this.getBiomedicalEquipmentById(equipmentId);
                                                                if (!session.user) {
                                                                    throw new Error("Equipment not found");
                                                                    ...
                                                                    for (let i = 1; i < correctiveMaintenances.length; i++) {
                                                                        ...
                                                                        if (!session.user) {
                                                                            ...
                                                                            exports._biomedicalService = new BiomedicalService();
                                                                        }
                                                                    }
                                                                }
                                                            }
