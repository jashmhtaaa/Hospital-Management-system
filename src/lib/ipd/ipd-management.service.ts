...
export const createAdmissionInDB = async (admission: Admission): Promise<Admission & { id: string }> => {
  const service = getIPDService();
  return service.createAdmission(admission);
};

export const updateAdmissionInDB = async (id: string, updates: Partial<Admission>): Promise<Admission> => {
  const service = getIPDService();
  return service.updateAdmission(id, updates);
};

export const dischargePatientFromDB = async (discharge: Discharge): Promise<Discharge & { id: string }> => {
  const service = getIPDService();
  return service.dischargePatient(discharge);
};

export const getBedAvailabilityFromDB = async (wardId?: string): Promise<BedAvailability[]> => {
  const service = getIPDService();
