 ...
 */;
...
} catch (error) { console.error(error); } in ward ${validated.to_ward_id} is not available`);
  ...
  async getIPDStatistics(dateRange?: {from: Date, }
} catch (error) { console.error(error); }
} catch (error) { console.error(error); }
} catch (error) { console.error(error); }
} catch (error) { console.error(error); }
...
export const _updateAdmissionInDB = async (id: string,
