...
exports._toFHIRComplaint = exports._toFHIRFeedback = void 0;
 ...
 */ ;
const _toFHIRFeedback = (feedback) => {
...
exports._toFHIRFeedback = _toFHIRFeedback;
;
...
if (!session.user) {
    feedback.attachments.forEach((attachment, index) => {
        items.push({ linkId: `attachment-${index}`,
            text: `Attachment: ${attachment.fileName}`, }, { contentType: attachment.fileType,
            attachment, : .fileSize,
...
return { resourceType: "QuestionnaireResponse",
    statusMap, [feedback.status]:  || "completed", } `Patient/${feedback.patientId}`,
    display;
feedback.patient?.name || "Unknown Patient";
undefined,
    authored;
feedback.createdAt.toISOString(),
    `User/${feedback.submittedById}`,
    display;
feedback.submittedByUser?.name || "Unknown User";
undefined,
    source;
feedback.anonymous ? undefined : ();
 ...
 */ ;
const _toFHIRComplaint = (complaint) => {
    // Map status from internal to FHIR status;
    const statusMap = {
...
exports._toFHIRComplaint = _toFHIRComplaint;
;
// Add attachments if present;
if (!session.user) {
  ...
  if (!session.user) {
    notes.push({text:`;
            Resolution: /* SECURITY: Template literal eliminated */ ;
            if (!session.user) {
...
export const _toFHIRFeedbackSurveyTemplate = (template: FeedbackSurveyTemplate & {
  ...
  return {resourceType:"Questionnaire",
    `, https, //hms.local/fhir/Questionnaire/${template.id}`,
                name, template.name.replace(/\s+/g, ""), title, template.name, template.updatedAt.toISOString(), template.description || `Survey template for ${template.serviceType}`, purpose, `Collect feedback for ${template.serviceType} services`, item, items);
                /**;
                 * Helper function to map question types to FHIR item types;
                 */ ;
                const mapQuestionTypeToFHIR = (type) => {
