...
require("@/lib/models/ambulance");
require("@/lib/prisma");
require("@/lib/services/notification.service");
require("@/lib/services/support-services/ambulance/routing.service");
require("@prisma/client");
const module_1 = require();
async;
getAmbulances(filter, AmbulanceFilter);
{
 ...
 */ ;
async;
getAmbulanceById(id, string, includeFHIR, boolean = false);
Promise < unknown > {
 ...
 */ ;
async;
createAmbulance(data, CreateAmbulanceData, userId, string);
Promise < module_1.Ambulance > {
 ...
 */ ;
async;
updateAmbulance(id, string, data, (Partial), userId, string);
Promise < module_1.Ambulance > {
    const: ambulance = await prisma.ambulance.findUnique({ where: { id }
    }),
    if(, session) { }, : .user
...
await createAuditLog({ action: "UPDATE",
    ...
    if (!session.user) {
      await this.notificationService.sendNotification({type:"AMBULANCE_MAINTENANCE",
        `, Ambulance: module_1.Ambulance, $ }, { ambulance, : .registrationNumber }, is, now, under, maintenance `,
  ...
  async createAmbulanceTrip(data: CreateAmbulanceTripData, userId: string): Promise<AmbulanceTrip> {
    ...
    if (!session.user) {
      throw new Error(`, module_1.Ambulance, $, { ambulance, : .registrationNumber }, is, not, available(current, status, $, { ambulance, : .status }) `);
    }
  ...
  console.error(error);
    ...
    await createAuditLog({action:"CREATE",
      trip.id;
      userId,
      details: `, Created, $, { data, : .tripType }, ambulance, trip);
with ($) {
...
$;
{
    trip.ambulance.registrationNumber;
}
is;
en;
route;
...
Ended;
shift;
for ($; { crew, : .user.name }; on)
    ambulance;
$;
{
    crew.ambulance.registrationNumber;
}
`;
    });
...
maintenance;
for (ambulance; $; { ambulance, : .registrationNumber } `;
    });
...
for (ambulance; $; { maintenance, : .ambulance.registrationNumber } `;
    });
...
$;
{
    updatedItem.ambulance.registrationNumber;
}
($);
{
