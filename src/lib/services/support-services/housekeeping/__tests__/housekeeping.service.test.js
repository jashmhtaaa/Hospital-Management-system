...
describe("HousekeepingService", () => {
    ...
    describe("getHousekeepingRequests", () => {
        it("should return housekeeping requests with pagination", async () => {
            ...
            }
            {
                ...
                module_2.prisma.housekeepingRequest.findMany.mockResolvedValue(mockRequests);
                module_2.prisma.housekeepingRequest.count.mockResolvedValue(2);
                // Call the service method;
                const result = await housekeepingService.getHousekeepingRequests({ page: 1,
                    limit: 10
                });
                // Verify Prisma was called with correct arguments;
                expect(module_2.prisma.housekeepingRequest.findMany).toHaveBeenCalledWith();
    ...
    it("should apply filters correctly", async () => {
        ...
        const mockRequests = [];
        {
            ...
            // Mock Prisma response;
            module_2.prisma.housekeepingRequest.findMany.mockResolvedValue(mockRequests);
            module_2.prisma.housekeepingRequest.count.mockResolvedValue(1);
...
describe("createHousekeepingRequest", () => {
    it("should create a new housekeeping request", async () => {
        ...
        module_2.prisma.housekeepingRequest.create.mockResolvedValue(mockCreatedRequest);
        // Call the service method;
        const result = await housekeepingService.createHousekeepingRequest(mockRequest);
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.housekeepingRequest.create).toHaveBeenCalledWith({
            "location1": ,
            "HIGH": ,
            expect, : .any(Date),
            "user1": ,
...
it("should throw an error if location does not exist", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.location.findUnique.mockResolvedValue(null);
...
describe("getHousekeepingRequestById", () => {
    it("should return a housekeeping request by ID", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(mockRequest);
        // Call the service method;
        const result = await housekeepingService.getHousekeepingRequestById("1");
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.housekeepingRequest.findUnique).toHaveBeenCalledWith({ where: { id: "1" },
    ...
    it("should throw an error if request does not exist", async () => {
        // Mock Prisma response;
        module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(null);
...
describe("updateHousekeepingRequest", () => {
    it("should update a housekeeping request", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.housekeepingRequest.update.mockResolvedValue(mockUpdatedRequest);
        // Call the service method;
        const result = await housekeepingService.updateHousekeepingRequest("1", mockUpdateData);
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.housekeepingRequest.update).toHaveBeenCalledWith({ where: { id: "1" },
            data: mockUpdateData,
            include: expect.any(Object)
        });
        // Verify result;
...
describe("assignHousekeepingRequest", () => {
    it("should assign a staff member to a housekeeping request", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.housekeepingStaff.findUnique.mockResolvedValue(mockStaff);
        module_2.prisma.housekeepingRequest.update.mockResolvedValue(mockUpdatedRequest);
        // Call the service method;
...
it("should throw an error if staff does not exist", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(mockExistingRequest);
    module_2.prisma.housekeepingStaff.findUnique.mockResolvedValue(null);
...
describe("deleteHousekeepingRequest", () => {
    it("should delete a housekeeping request", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.housekeepingRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.housekeepingRequest.delete.mockResolvedValue(mockExistingRequest);
        // Call the service method;
        await housekeepingService.deleteHousekeepingRequest("1");
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.housekeepingRequest.delete).toHaveBeenCalledWith({ where: { id: "1" }
        });
    });
    it("should throw an error if request does not exist", async () => {
        // Mock Prisma response;
...
describe("getHousekeepingAnalytics", () => {
    it("should return analytics data", async () => {
        ...
        {
            requestType: "CLEANING", count;
            12;
        }
        {
            requestType: "DISINFECTION", count;
            5;
        }
        {
            requestType: "LINEN_CHANGE", count;
            3;
        }
        {
            requestType: "WASTE_DISPOSAL", count;
            1;
        }
        ;
        ...
        module_2.prisma.housekeepingRequest.groupBy = vi.fn();
        module_2.prisma.housekeepingRequest.groupBy;
        mockResolvedValueOnce(mockStatusCounts);
        mockResolvedValueOnce(mockRequestTypes);
        mockResolvedValueOnce(mockPriorities);
        module_2.prisma.housekeepingRequest.count.mockResolvedValue(21);
        // Call the service method;
...
it("should apply date filters when provided", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.housekeepingRequest.groupBy = vi.fn().mockResolvedValue([]);
    module_2.prisma.housekeepingRequest.count.mockResolvedValue(0);
    ...
    expect(module_2.prisma.housekeepingRequest.count).toHaveBeenCalledWith({}, { gte: fromDate,
        lte: toDate
    }),
        expect(module_2.prisma.housekeepingRequest.groupBy).toHaveBeenCalledWith();
