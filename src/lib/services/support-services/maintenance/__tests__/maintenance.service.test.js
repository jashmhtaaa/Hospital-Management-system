...
describe("MaintenanceService", () => {
    ...
    describe("getMaintenanceRequests", () => {
        it("should return maintenance requests with pagination", async () => {
            ...
            }
            {
                ...
                module_2.prisma.maintenanceRequest.findMany.mockResolvedValue(mockRequests);
                module_2.prisma.maintenanceRequest.count.mockResolvedValue(2);
                // Call the service method;
                const result = await maintenanceService.getMaintenanceRequests({ page: 1,
                    limit: 10
                });
                // Verify Prisma was called with correct arguments;
                expect(module_2.prisma.maintenanceRequest.findMany).toHaveBeenCalledWith();
    ...
    it("should apply filters correctly", async () => {
        ...
        const mockRequests = [];
        {
            ...
            // Mock Prisma response;
            module_2.prisma.maintenanceRequest.findMany.mockResolvedValue(mockRequests);
            module_2.prisma.maintenanceRequest.count.mockResolvedValue(1);
    ...
    it("should create a new maintenance request", async () => {
        ...
        const mockRequest = { assetId: "asset1",
            ...
            const: mockCreatedRequest = { id: "1",
                ...
                createdAt: new Date(),
                updatedAt: new Date()
            } }(module_2.prisma.maintenanceAsset.findUnique).mockResolvedValue({ id: "asset1", name: "AC Unit 101" });
        module_2.prisma.user.findUnique.mockResolvedValue({ id: "user1", name: "John Doe" });
        module_2.prisma.department.findUnique.mockResolvedValue({ id: "dept1", name: "Cardiology" });
        module_2.prisma.maintenanceRequest.create.mockResolvedValue(mockCreatedRequest);
        // Call the service method;
        const result = await maintenanceService.createMaintenanceRequest(mockRequest);
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.maintenanceRequest.create).toHaveBeenCalledWith({
            "asset1": ,
            "HIGH": ,
            expect, : .any(Date),
            "user1": ,
...
it("should throw an error if asset does not exist", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.maintenanceAsset.findUnique.mockResolvedValue(null);
...
describe("getMaintenanceRequestById", () => {
    it("should return a maintenance request by ID", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockRequest);
        // Call the service method;
        const result = await maintenanceService.getMaintenanceRequestById("1");
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.maintenanceRequest.findUnique).toHaveBeenCalledWith({ where: { id: "1" },
    ...
    it("should throw an error if request does not exist", async () => {
        // Mock Prisma response;
        module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(null);
...
describe("updateMaintenanceRequest", () => {
    it("should update a maintenance request", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.maintenanceRequest.update.mockResolvedValue(mockUpdatedRequest);
        // Call the service method;
        const result = await maintenanceService.updateMaintenanceRequest("1", mockUpdateData);
        // Verify Prisma was called with correct arguments;
        expect(module_2.prisma.maintenanceRequest.update).toHaveBeenCalledWith({ where: { id: "1" },
            data: mockUpdateData,
            include: expect.any(Object)
        });
        // Verify result;
...
describe("assignMaintenanceRequest", () => {
    it("should assign a technician to a maintenance request", async () => {
        ...
        // Mock Prisma response;
        module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.maintenanceTechnician.findUnique.mockResolvedValue(mockTechnician);
        module_2.prisma.maintenanceRequest.update.mockResolvedValue(mockUpdatedRequest);
        // Call the service method;
...
it("should throw an error if technician does not exist", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockExistingRequest);
    module_2.prisma.maintenanceTechnician.findUnique.mockResolvedValue(null);
...
describe("completeMaintenanceRequest", () => {
    it("should mark a maintenance request as completed", async () => {
        ...
        const mockPartsUsed = [];
        {
            partId: "part1", quantity;
            2;
        }
        {
            partId: "part2", quantity;
            1;
        }
        ;
        const mockUpdatedRequest = {
        ...
        module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockExistingRequest);
        module_2.prisma.maintenanceTechnician.findUnique.mockResolvedValue(mockTechnician);
        module_2.prisma.maintenancePart.findUnique;
        ...
        module_2.prisma.maintenancePart.update.mockResolvedValue({});
        module_2.prisma.maintenanceRequest.update.mockResolvedValue(mockUpdatedRequest);
        // Call the service method;
...
expect(module_2.prisma.maintenancePart.update).toHaveBeenCalledTimes(2),
    expect(module_2.prisma.maintenancePart.update).toHaveBeenCalledWith({ where: { id: "part1" },
        data: { currentStock: 8 } // 10 - 2;
    }),
    expect(module_2.prisma.maintenancePart.update).toHaveBeenCalledWith({ where: { id: "part2" },
...
it("should throw an error if insufficient parts stock", async () => {
    ...
    module_2.prisma.maintenanceRequest.findUnique.mockResolvedValue(mockExistingRequest);
    module_2.prisma.maintenanceTechnician.findUnique.mockResolvedValue(mockTechnician);
    module_2.prisma.maintenancePart.findUnique.mockResolvedValue({ id: "part1",
...
describe("getMaintenanceAssets", () => {
    it("should return maintenance assets with pagination", async () => {
        ...
        }
        {
            ...
            ;
            // Mock Prisma response;
            module_2.prisma.maintenanceAsset.findMany.mockResolvedValue(mockAssets);
            module_2.prisma.maintenanceAsset.count.mockResolvedValue(2);
            // Call the service method;
            const result = await maintenanceService.getMaintenanceAssets({ page: 1,
                limit: 10
            });
            // Verify Prisma was called with correct arguments;
            expect(module_2.prisma.maintenanceAsset.findMany).toHaveBeenCalledWith();
            expect.objectContaining({ skip: 0,
                "asc": 
            });
        }
...
it("should apply filters correctly", async () => {
    ...
    const mockAssets = [];
    {
        ...
        // Mock Prisma response;
        module_2.prisma.maintenanceAsset.findMany.mockResolvedValue(mockAssets);
        module_2.prisma.maintenanceAsset.count.mockResolvedValue(1);
        // Call the service method with filters;
...
describe("getMaintenanceAnalytics", () => {
    it("should return analytics data", async () => {
        ...
        {
            requestType: "PREVENTIVE", count;
            8;
        }
        {
            requestType: "CORRECTIVE", count;
            10;
        }
        {
        ...
        module_2.prisma.maintenanceRequest.groupBy = vi.fn();
        module_2.prisma.maintenanceRequest.groupBy;
        mockResolvedValueOnce(mockStatusCounts);
        mockResolvedValueOnce(mockRequestTypes);
        mockResolvedValueOnce(mockPriorities);
        module_2.prisma.maintenanceRequest.count.mockResolvedValue(21);
        // Call the service method;
...
it("should apply date filters when provided", async () => {
    ...
    // Mock Prisma response;
    module_2.prisma.maintenanceRequest.groupBy = vi.fn().mockResolvedValue([]);
    module_2.prisma.maintenanceRequest.count.mockResolvedValue(0);
    ...
    expect(module_2.prisma.maintenanceRequest.count).toHaveBeenCalledWith({}, { gte: fromDate,
        lte: toDate
    }),
        expect(module_2.prisma.maintenanceRequest.groupBy).toHaveBeenCalledWith();
