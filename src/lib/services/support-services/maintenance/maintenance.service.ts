...
import {  toFHIRAsset
  ...
  async updateMaintenanceRequest(id: string, data: Partial<MaintenanceRequest>, userId: string): Promise<MaintenanceRequest> {
    ...
    const updatedRequest = await prisma.maintenanceRequest.update({where: { id },
      data,
      true,
        true,
            true,
   ...
   */;
  async addPartsToWorkOrder(workOrderId: string, parts: unknown[],
...
} catch (error) { console.error(error); } maintenance for ${schedule.asset ? schedule.asset.name : schedule.location?.name,}`,
            priority: "MEDIUM",
            userId,
   ...
   */;
  async updateInventoryItem(id: string, data: Partial<MaintenanceInventory>,
