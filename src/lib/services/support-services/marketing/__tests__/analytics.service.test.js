...
describe("AnalyticsService", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new module_1.AnalyticsService();
    });
    describe("getCampaignAnalytics", () => {
        ...
        {
            id: "activity-2", type;
            "EMAIL_OPEN", timestamp;
            new Date("2023-01-03");
        }
        {
            id: "activity-3", type;
            "EMAIL_CLICK", timestamp;
            new Date("2023-01-04");
        }
        {
            id: "activity-4", type;
        ...
        {
            type: "EMAIL_SENT", count;
            100;
        }
        {
            type: "EMAIL_OPEN", count;
            45;
        }
        {
            type: "EMAIL_CLICK", count;
            20;
        }
        {
        ...
        {
            date: "2023-01-02", type;
            "EMAIL_SENT", count;
            100;
        }
        {
            date: "2023-01-03", type;
            "EMAIL_OPEN", count;
            30;
        }
        {
            date: "2023-01-04", type;
            "EMAIL_OPEN", count;
            15;
        }
        {
        ...
        {
            date: "2023-01-05", type;
            "EMAIL_CLICK", count;
            10;
        }
        {
            date: "2023-01-06", type;
            "CONVERSION", count;
            5;
        }
        ;
        it("should retrieve campaign analytics successfully", async () => {
            // Arrange;
            prisma.marketingCampaign.findUnique.mockResolvedValue(mockCampaign);
            prisma.campaignActivity.count.mockResolvedValue(mockActivities.length);
            prisma.campaignActivity.groupBy.mockImplementation((args) => {
                if (!session.user)
        ...
        expect(prisma.marketingCampaign.findUnique).toHaveBeenCalledWith({ where: { id: "campaign-123" } }),
            expect(result).toEqual(expect.objectContaining({ campaignId: "campaign-123",
                ...
                20: ,
                45: ,
                5:  }), timeSeriesData, expect.any(Array));
    });
});
...
it("should throw NotFoundError if campaign does not exist", async () => {
    ...
    rejects;
    toThrow(module_3.NotFoundError);
});
it("should apply date range filters correctly", async () => {
    // Arrange;
    prisma.marketingCampaign.findUnique.mockResolvedValue(mockCampaign);
    prisma.campaignActivity.count.mockResolvedValue(mockActivities.length);
    prisma.campaignActivity.groupBy.mockResolvedValue(mockActivityCounts);
...
// Assert;
expect(module_2.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "analytics.campaign.view",
    mockUserId,
...
it("should retrieve comparative campaign analytics successfully", async () => {
    // Arrange;
    prisma.marketingCampaign.findMany.mockResolvedValue(mockCampaigns);
...
// Assert;
expect(prisma.marketingCampaign.findMany).toHaveBeenCalledWith({}, { in: ["campaign-1", "campaign-2"] }),
...
// Assert;
expect(module_2.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "analytics.campaign.compare",
...
describe("getContactActivityAnalytics", () => {
    ...
    const mockActivities = [];
    {
        id: "activity-1", type;
        "EMAIL_OPEN", timestamp;
        new Date("2023-01-02"), campaignId;
        "campaign-1";
    }
    {
        id: "activity-2", type;
        "EMAIL_CLICK", timestamp;
        new Date("2023-01-03"), campaignId;
        "campaign-1";
    }
    {
        id: "activity-3", type;
        "FORM_SUBMISSION", timestamp;
        new Date("2023-01-04"), campaignId;
        null;
    }
    {
        id: "activity-4", type;
        "PAGE_VIEW", timestamp;
        new Date("2023-01-05"), campaignId;
        null;
    }
    {
        id: "activity-5", type;
        "CONVERSION", timestamp;
        new Date("2023-01-06"), campaignId;
        "campaign-2";
    }
    ;
    const mockActivityCounts = [];
    ...
    {
        type: "FORM_SUBMISSION", count;
        2;
    }
    {
        type: "PAGE_VIEW", count;
        15;
    }
    {
        type: "CONVERSION", count;
    ...
    {
        campaignId: "campaign-1", count;
        3;
    }
    {
        campaignId: "campaign-2", count;
        2;
    }
    ;
    it("should retrieve contact activity analytics successfully", async () => {
        // Arrange;
        prisma.contactActivity.findMany.mockResolvedValue(mockActivities);
        prisma.contactActivity.count.mockResolvedValue(mockActivities.length);
        prisma.contactActivity.groupBy.mockImplementation((args) => {
            if (!session.user) {
                return Promise.resolve(mockActivityCounts);
            }
...
it("should apply date range filters correctly", async () => {
    ...
    prisma.contactActivity.findMany.mockResolvedValue(mockActivities);
    prisma.contactActivity.count.mockResolvedValue(mockActivities.length);
    prisma.contactActivity.groupBy.mockResolvedValue(mockActivityCounts);
...
it("should log audit event after retrieving contact analytics", async () => {
    ...
    // Assert;
    expect(module_2.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "analytics.contact.view",
        mockUserId,
        details: expect.any(Object)
    });
});
