...
describe("SegmentService", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new module_3.SegmentService();
    });
    ...
    it("should create a segment successfully", async () => {
        // Arrange;
        prisma.segment.create.mockResolvedValue(mockCreatedSegment);
        // Act;
...
it("should throw DatabaseError if database operation fails", async () => {
    // Arrange;
    prisma.segment.create.mockRejectedValue();
    // Act & Assert;
...
it("should log audit event after creating segment", async () => {
    ...
    // Assert;
    expect(module_1.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "segment.create",
        mockUserId,
...
it("should retrieve a segment by ID", async () => {
    // Arrange;
    prisma.segment.findUnique.mockResolvedValue(mockSegment);
    // Act;
...
describe("getSegments", () => {
    ...
    it("should retrieve segments with pagination", async () => {
        // Arrange;
        prisma.segment.count.mockResolvedValue(2);
        prisma.segment.findMany.mockResolvedValue(mockSegments);
        // Act;
...
it("should apply filters correctly", async () => {
    ...
    prisma.segment.count.mockResolvedValue(10);
    prisma.segment.findMany.mockResolvedValue([mockSegments[0]]);
    // Act;
...
it("should log audit event after updating segment", async () => {
    ...
    // Assert;
    expect(module_1.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "segment.update",
        mockUserId,
...
describe("addContactToSegment", () => {
    ...
    it("should add a contact to a segment successfully", async () => {
        ...
        prisma.contactSegment.findUnique.mockResolvedValue(null); // Contact not already in segment;
        prisma.contactSegment.create.mockResolvedValue(mockContactSegment);
        // Act;
    ...
    expect(prisma.segment.findUnique).toHaveBeenCalledWith({ where: { id: "segment-123" } }),
        ...
        expect(prisma.contactSegment.findUnique).toHaveBeenCalledWith({}, { segmentId: "segment-123",
...
it("should return existing record if contact is already in segment", async () => {
    ...
    prisma.segment.findUnique.mockResolvedValue(mockSegment);
    prisma.contact.findUnique.mockResolvedValue(mockContact);
    prisma.contactSegment.findUnique.mockResolvedValue(mockContactSegment); // Contact already in segment;
    // Act;
    const result = await service.addContactToSegment();
    "segment-123",
        "contact-123",
...
it("should log audit event after adding contact to segment", async () => {
    ...
    // Assert;
    expect(module_1.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "segment.contact.add",
        mockUserId,
        "contact-123":  });
});
;
...
describe("removeContactFromSegment", () => {
    ...
    it("should remove a contact from a segment successfully", async () => {
        ...
        prisma.segment.findUnique.mockResolvedValue(mockSegment);
        prisma.contact.findUnique.mockResolvedValue(mockContact);
        prisma.contactSegment.findUnique.mockResolvedValue(mockContactSegment);
        prisma.contactSegment.delete.mockResolvedValue(mockContactSegment);
        // Act;
    ...
    expect(prisma.contactSegment.delete).toHaveBeenCalledWith({}, { segmentId: "segment-123",
        contactId: "contact-123"
    }),
...
it("should log audit event after removing contact from segment", async () => {
    ...
    // Assert;
    expect(module_1.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "segment.contact.remove",
        mockUserId,
        "contact-123":  });
});
;
...
it("should apply criteria and add matching contacts to segment", async () => {
    // Arrange;
    prisma.segment.findUnique.mockResolvedValue(mockSegment);
    prisma.contact.findMany.mockResolvedValue(mockMatchingContacts);
    prisma.contactSegment.findMany.mockResolvedValue([]);
    {
        contactId: "contact-1", segmentId;
        "segment-123";
...
// Assert;
expect(module_1.AuditLogger.prototype.log).toHaveBeenCalledWith({ action: "segment.criteria.apply",
    mockUserId,
    2: ,
    addedCount: 1 });
;
