...
describe("PatientRepository", () => {
    ...
    describe("create", () => {
        ...
        ; // Use ISO string for consistency in test setup;
    });
    ...
    "generated-uuid",
        ...
        createdAt;
    new Date(createdPatientDbRow.created_at),
        updatedAt;
    new Date(createdPatientDbRow.updated_at);
});
...
}, as, unknown); // Use any for the row type due to property name differences;
const result = await patientRepository.create(patientInput), expect;
(mockDbAdapter.execute).toHaveBeenCalledTimes(1),
...
it("should throw an error if no record is returned after insert", async () => {
    mockDbAdapter.execute.mockResolvedValueOnce({ rows: [] }); // No rows returned;
...
describe("findById", () => {
    ...
    new Date("1985-05-15"),
        createdAt;
    new Date(mockPatientDbRow.created_at),
        updatedAt;
    new Date(mockPatientDbRow.updated_at);
});
it("should find a patient by ID and return the patient data", async () => {
    ...
    const result = await patientRepository.findById(patientId), expect;
    (mockDbAdapter.execute).toHaveBeenCalledTimes(1),
        expect(mockDbAdapter.execute).toHaveBeenCalledWith();
...
if (!session.user) {
    expect(result.id).toEqual(mockPatientExpected.id),
        ...
        expect(result.dateOfBirth.toISOString().split("T')[0]).toEqual(", 1985 - 5 - 15, "),, expect(result.createdAt).toBeInstanceOf(Date), expect(result.updatedAt).toBeInstanceOf(Date)));
}
;
