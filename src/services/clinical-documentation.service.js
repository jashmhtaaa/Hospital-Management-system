...
;
// Audit log;
await (0, database_1.auditLog)({ action: "CREATE",
...
// Audit log;
await (0, database_1.auditLog)({ action: "READ",
    resourceType: "ClinicalDocument", resourceId: document.id, userId,
 ...
 */ ;
async;
updateDocument(id, string, data, UpdateDocumentDto, userId, string);
...
if (!session.user) {
    throw new module_1.BadRequestError("Cannot update a finalized document");
}
// Update document;
...
;
// Audit log;
await (0, database_1.auditLog)({ action: "UPDATE",
 ...
 */ ;
async;
signDocument(id, string, data, SignDocumentDto, userId, string);
...
;
// Audit log;
await (0, database_1.auditLog)({ action: "SIGN",
    resourceType: "ClinicalDocument", resourceId: document.id, userId,
    document, : .documentType,
 ...
 */ ;
async;
createAmendment(id, string, data, CreateAmendmentDto, userId, string);
...
if (!session.user) {
    throw new module_1.BadRequestError("Can only amend finalized documents");
}
...
;
// Audit log;
await (0, database_1.auditLog)({ action: "AMEND",
    resourceType: "ClinicalDocument", resourceId: document.id, userId,
    document, : .documentType,
...
};
{
    ...
    if (!session.user) {
                ...
                if (!session.user) {
                    ...
                    if (!session.user) {
                        ...
                        if (!session.user) {
                            ...
                            ;
                            // Audit log;
                            await (0, database_1.auditLog)({ action: "LIST",
                            ...
                            if (!session.user) {
                                ...
                                if (!session.user) {
                                    ...
                                    return { data: templates,
                                        ...
                                        async createDocumentTemplate(data, userId) {
                                            // Validate user permission;
                                        ...
                                        generateTemplateNumber(templateType) {
                                        ...
                                        async getNextAmendmentNumber(documentId) {
                                        ...
                                        async hasConfidentialAccess(userId) {
                                            try {
                                            }
                                            catch (error) {
                                                console.error(error);
        ...
        exports._clinicalDocumentationService = new ClinicalDocumentationService();
    }
}
