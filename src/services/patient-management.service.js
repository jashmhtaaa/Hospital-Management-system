...
require("../lib/fhir/fhir-r4-base");
require("../lib/notifications");
require("./audit_log_service.ts");
require("./auth_service.ts");
...
const AddressSchema = z.object({ addressType: z.string().min(1, "Address type is required"),
    ...
    validFrom: z.date().default(validTo, z.date().optional(), isBilling, z.boolean().default(false), isShipping, z.boolean().default(false), notes, z.string().optional())
});
...
Promise < string > {
    ...
    const: patientCount = await prisma.patient.count({}, { gte: today
    })
};
;
 ...
 */ ;
async;
createPatient(patientData, unknown, userId, string);
Promise < Patient > {
    try: {}, catch(error) {
        console.error(error);
    }
 ...
 */ ;
async;
updatePatient(patientId, string, patientData, unknown, userId, string);
Promise < Patient > {
    try: {}, catch(error) {
        console.error(error);
    }
 ...
 */ ;
async;
addPatientAddress(patientId, string, addressData, unknown, userId, string);
 ...
 */ ;
async;
addPatientIdentification(patientId, string, identificationData, unknown, userId, string);
Promise < PatientIdentification > {
    try: {}, catch(error) {
        console.error(error);
    }
};
 ...
 */ ;
async;
addEmergencyContact(patientId, string, contactData, unknown, userId, string);
Promise < module_1.EmergencyContact > {
    try: {}, catch(error) {
        console.error(error);
    }
};
...
if (!session.user) {
    ...
    try { }
         ...
         */ ;
        async;
        addInsurance(patientId, string, insuranceData, unknown, userId, string);
        ...
        if (!session.user) {
            ...
            if (!session.user) {
                ...
                try { }
                     ...
                     */ ;
                    async;
                    searchPatients(searchParams, unknown, userId, string);
                    Promise < unknown > {
                        try: {}, catch(error) {
                            console.error(error);
                        }
    ...
    if (!session.user) {
                ...
                if (!session.user) {
                    ...
                    if (!session.user) {
                        ...
                        try { }
                             ...
                             */ ;
                            async;
                            markPatientDeceased(patientId, string, data, { deceasedDate: Date, deceasedReason: string }, userId, string);
                            Promise < Patient > {
                                try: {}, catch(error) {
                                    console.error(error);
                                }
        ...
        try { }
            ...
            try { }
                 ...
                 */ ;
                async;
                mergePatients(sourcePatientId, string, targetPatientId, string, userId, string);
                Promise < Patient > {
                    try: {}, catch(error) {
                        console.error(error);
                    }
                    ...
                    if (!session.user) {
                        ...
                        for (const contact of sourcePatient.contacts) {
                            ...
                            try { }
                                 ...
                                 */ ;
                                async;
                                getPatientMPI(patientId, string, userId, string);
                                Promise < unknown > {
                                    try: {}, catch(error) {
                                        console.error(error);
                                    }
